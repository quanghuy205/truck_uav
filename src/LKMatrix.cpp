...
LKMatrix::LKMatrix(vector <vector<double> > VehicleCost, vector<int> &ids) {
  ...
  for (int i = 0; i < size; i++) {
    for (int j = 0; j < size; j++) {
      edgeDistances[i][j] = VehicleCost[i][j];
      edgeDistances[j][i] = VehicleCost[i][j];
    }
  }
}
...
void LKMatrix::printTourIds(vector <int> &T_opt_vehicle) {
  int current = 0;
  do {
    cout << ids[current] << " ";
    T_opt_vehicle.push_back(ids[current]);
    current = tour[current];
  } while (current != 0);
}
